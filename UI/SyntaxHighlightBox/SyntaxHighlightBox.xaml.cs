	...
	public partial class SyntaxHighlightBox : TextBox 
    {
        ...
        public double LineHeight
        {
            ...
            set
            {
                ...
                {
                    lineHeight = value;
                    blockHeight = MaxLinesInBlock * value;
        ...
        public int MaxLinesInBlock
        {
            get 
            { 
                return maxLinesInBlock; 
            }
            set
            {
                maxLinesInBlock = value > 0 ? value : 0;
        ...
        public SyntaxHighlightBox()
        {
            ...
            Loaded += (s, e) =>
            {
                ...
                lineNumbersCanvas.Width = GetFormattedTextWidth(string.Format("{0:0000}", linesTotal)) + 5;
        ...
        {
			linesTotal = TextUtilities.GetLinesQty(Text);
		}
		...
		private void UpdateBlocks() 
        {
			...
			while (!blocks.Last().IsLast && blocks.Last().Position.Y + blockHeight - VerticalOffset < ActualHeight) 
            {
				...
				int lastLineIndex = firstLineIndex + maxLinesInBlock - 1;
				lastLineIndex = lastLineIndex <= linesTotal - 1 ? lastLineIndex : linesTotal - 1;

				int fisrCharIndex = blocks.Last().CharEndIndex + 1;
				int lastCharIndex = TextUtilities.GetLastCharIndex(Text, lastLineIndex); // to be optimized (forward search)
		...
		private void InvalidateBlocks(int changeOffset) 
        {
			...
			int lvchar = TextUtilities.GetLastCharIndex(Text, lvline);
			...
			for (int i = fvchar; i < Text.Length; i++) 
            {
				...
				if (i == Text.Length - 1) 
                {
					...
					InnerTextBlock block = new InnerTextBlock(
						charStart,
						i, lineStart,
						lineStart + TextUtilities.GetLinesQty(blockText) - 1,
						LineHeight);
					block.RawText = block.GetSubString(Text);
					block.LineNumbers = GetFormattedLineNumbers(block.LineStartIndex, block.LineEndIndex);
					block.IsLast = true;
				...
				if (localLineCount > maxLinesInBlock) 
                {
					InnerTextBlock block = new InnerTextBlock(
						charStart,
						i,
						lineStart,
						lineStart + maxLinesInBlock - 1,
						LineHeight);
					block.RawText = block.GetSubString(Text);
					block.LineNumbers = GetFormattedLineNumbers(block.LineStartIndex, block.LineEndIndex);
		...
		public int GetIndexOfFirstVisibleLine() {
			...
			return guessedLine > linesTotal ? linesTotal : guessedLine;
		}
		...
		public int GetIndexOfLastVisibleLine() {
			...
			return guessedLine > linesTotal - 1 ? linesTotal - 1 : guessedLine;
		}
